...
public static class Base16384 {
    ...
    public static ulong Htobe64(ulong value) =>
        BitConverter.IsLittleEndian ? ReverseEndian(value) : value;

    public static ulong Be64toh(ulong value) =>
        BitConverter.IsLittleEndian ? ReverseEndian(value) : value;


    public static unsafe int Base16384_encode(byte* data, int dlen, byte* buf, int blen) {
        var outlen = dlen / 7 * 8;
        var offset = dlen % 7;
        ...
        for (; i <= dlen - 7; i += 7) {
            ulong sum = 0;
            var shift = Htobe64(*(ulong*)(data + i)) >> 2; // 这里有读取越界
    ...
    public static unsafe int Base16384_decode(byte* data, int dlen, byte* buf, int blen) {
        var outlen = dlen;
        var offset = 0;
        if (data[dlen - 2] == '=') {
            offset = data[dlen - 1];
        ...
        for (; i <= outlen - 7; n++, i += 7) {
            ulong sum = 0;
            var shift = Htobe64(vals[n]) - 0x4e004e004e004e00;
            shift <<= 2;
